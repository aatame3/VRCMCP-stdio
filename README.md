...
具体的に VRChat が禁止／要求している主な事項：

- **過剰なリクエストの禁止**：適切にキャッシュし、HTTP 429 を受け取ったら必ず後退（バックオフ）すること。固定の時計間隔（毎分 0 秒など）でのポーリングも禁止。
- **User-Agent による識別**：`applicationName/Version contactInfo` 形式で正しく自己申告すること（本ツールは現在 `VRCMCP/1.0.0` のみで連絡先を含んでいません。商用・広範な配布には不向きです）。
- **代理操作の禁止**：他人のアカウントで操作したり、他人の認証情報を要求・保管しないこと。
- **公式サポートなし**：非公式アプリ経由の API 利用について VRChat はサポートを提供しません。

本ツールは「個人が自分のアカウントで、常識的な頻度で自分のフレンド状況を確認する」用途を想定しています。**bot 化・自動巡回・第三者へのサービス提供・スクレイピング等の用途には使わないでください。** 上記ガイドラインに反する使い方をすると、**最悪の場合アカウント BAN のリスクがあります。**
